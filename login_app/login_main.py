...
import os
import login_app.data_handler as data_handler
import logging
import login_app.encrypt as encrypt
import time
...
from login_app.signup import form_signup #imports the signup function
from login_app.login import form_login
from login_app.forgot_password import forgot_password, reset_security_questions, new_password_process
from login_app.user_dashboard import user_dash
from login_app.admin_dashboard import admin_dash, verify_admin_pin
from login_app.user_change import user_edit, user_delete
from login_app.rate_limit import init_limiter, handle_rate_limit_error
from login_app.lock_manager import lock_manager
from login_app.url_validation import safe_redirect, is_safe_url

login_main_blue = Blueprint('main', __name__,
...
# login_main_blue.secret_key = secret_key_
# login_main_blue.config['MAX_CONTENT_LENGTH'] = 5 * 1024 * 1024  # Limit to 5 MB
# login_main_blue.config.update(
...
# )
# csrf = CSRFProtect(login_main_blue)  # Enable CSRF protection

# Initialise Flask Limiter
# limiter = init_limiter(login_main_blue)
...
def redirect_directions():
    ...
    if request.endpoint not in ['signup', 'password_forgot', 'new_password', 'reset', 'process_new_password', 'sq', 'index']:
        ...
        target = url_for('main.signup')
    elif request.endpoint in ['password_forgot', 'new_password', 'reset', 'process_new_password', 'sq']:
        target = url_for('main.password_forgot')
    else:
        target = url_for('index')
    return redirect(safe_redirect(target))


# @login_main_blue.before_request # This function runs before every request
# def before_request():
...
# @login_main_blue.route("/index", methods=["GET"])
# @login_main_blue.route("/index.htm", methods=["GET"])
# @login_main_blue.route("/index.html", methods=["GET"])
# @login_main_blue.route("/index.asp", methods=["GET"]) # Active server page
# @login_main_blue.route("/index.php", methods=["GET"])
# def root():
#     return redirect("/", 302)

# @login_main_blue.route("/")
# @csp_header(
#     {
...
@login_main_blue.route("/forgot_password", methods=["POST", "GET"])
def password_forgot():
    return forgot_password() # function to handle forgot password logic

@login_main_blue.route("/forgot_password_sq", methods=["POST"])
def sq():
    if not session.get("forgot_password_verified"):  
        abort(403)
    return reset_security_questions()

@login_main_blue.route("/new_password", methods=["POST", "GET"])
def new_password():
...
@login_main_blue.route("/process_new_password", methods=["POST"])
def process_new_password():
    return new_password_process()

@login_main_blue.route("/dashboard", methods=["GET"]) #changed from user_dashboard. Make it method get?
def user_dashboard():
    return user_dash()

@login_main_blue.route("/admin_dashboard", methods=["GET", "POST"]) #make it so only admin can acces
def admin_dashboard():    
    return admin_dash()

@login_main_blue.route("/admin_verify", methods=["GET", "POST"]) # Admin verification route
def admin_verify():
    if not session.get('pending_admin'): # Check if the user is pending admin
        return redirect(url_for('main.login'))
    return verify_admin_pin()

@login_main_blue.route("/user/<int:user_id>/edit", methods=["GET", "POST"]) # Edit user route
# @limiter.limit("5 per minute") # Prevent mass editing
def edit_user(user_id):
    return user_edit(user_id)

@login_main_blue.route("/delete_user/<int:user_id>", methods=["POST"])
# @limiter.limit("2 per minute") # Prevent mass deletion
def delete_user(user_id):
    return user_delete(user_id)

@login_main_blue.route("/release_lock/<int:user_id>", methods=["POST"])
...
@login_main_blue.errorhandler(400) # Bad Request
...
@login_main_blue.errorhandler(401) # Unauthorised
...
@login_main_blue.errorhandler(403) # Forbidden
...
@login_main_blue.errorhandler(404) # Not Found
def page_not_found(e):
    app_log.error(f"Page not found: {request.path} by {request.remote_addr}")
    return render_template('404.html'), 404

@login_main_blue.errorhandler(405) # Method Not Allowed
...
@login_main_blue.errorhandler(413) # Payload Too Large
...
@login_main_blue.errorhandler(429) # Too Many Requests
@login_main_blue.errorhandler(RateLimitExceeded) # Flask Specfic Limiter Error
...
@login_main_blue.errorhandler(500) # Internal Server Error
def internal_server_error(e):
    app_log.error(f"Internal server error: {e}")
    return render_template('500.html'), 500

@login_main_blue.errorhandler(Exception) # Unhandled Exception
