    ...
    try:
        if request.method == "GET":
            if session.get('Username_Login'):
                ...
                return redirect(url_for("main.user_dashboard"))  # Redirect to the dashboard or home page
        ...
        if request.method == "POST":
            ...
            if login_success: # True
                ...
                session['user_id'] = data_handler.get_user_id(username)  # Store user ID in session
            ...
            if session.get("checkout_redirect"):
                if session.get("checkout_redirect") == True:
                    session.pop("checkout_redirect", None) # Clear the checkout redirect flag
                    return redirect(url_for("checkout")) # Redirect to checkout page

            return redirect(url_for('main.user_dashboard')) # Redirect to a success page
