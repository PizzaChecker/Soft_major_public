...
from login_app.miscellaneous_security import random_delay
import logging

DATABASE_PATH = os.path.join(os.path.dirname(__file__), 'database.db')
UPLOAD_FOLDER = os.path.join(os.path.dirname(__file__),'static/uploads')  # Directory to save uploaded files
...
def is_user_unique(username: str) -> bool:
    connection = None
    try:
        connection = sqlite3.connect(DATABASE_PATH) # Establish a connection to the SQLite database
...
def get_user_id(username):
    ...
    try:
        ...
        if user_id:
            logger.info(f"Retrieved user ID for username: {username}")
            return user_id[0]  # Return the ID of the user
        return None  # Return None if the user is not found
    except sqlite3.OperationalError as e:
        logger.critical(f"Database operation error while retrieving user ID: {str(e)}", exc_info=True)
        return None
    except sqlite3.DatabaseError as e:
        logger.critical(f"Database error while retrieving user ID: {str(e)}", exc_info=True)
        return None
    except Exception as e:
        logger.critical(f"Unexpected error while retrieving user ID: {str(e)}", exc_info=True)
        return None
    finally:
        if connection:
            try:
